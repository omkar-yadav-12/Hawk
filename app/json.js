...
async function teamResults() {
    let allTeams = fs.readFileSync('app/apiData/teams/allTeamsResults.json')
    let data = JSON.parse(allTeams)
    let array = []
    ...
    fs.writeFile("app/apiData/teams/allTeamsResults.json", JSON.stringify(array), (err) => {
        if (err) console.error(err)
        else console.log("COMPLETE")
    })
}
...
async function updateMatches() {
    ...
    fs.writeFile("app/apiData/matches/allMatches.json", JSON.stringify(iowaMatches), (err) => {
        if (err) console.error(err)
        else console.log("COMPLETE")
    })
